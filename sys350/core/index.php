...
//define('CONTROLLER_PATH', PATH_67 . '/controller');
//缓存目录路径CACHE_67
...
$domain = !empty($config['domains'][$host]) ?
    $config['domains'][$host] : $config['domains']['']/*默认配置*/;
//域名常量DOMAIN_67
defined('DOMAIN_67') or define('DOMAIN_67', $domain['domain']);

//子系统标识常量SYSTEM_67
define('SYSTEM_67', 'user');
//文档地址常量DOC_67
define('DOC_67', $config['doc']);
...
//将配置信息的域名信息赋值给公共类(容器类)C67的变量$domain
C67::$domain = $domain['domain'];
...
//配置cookies服务器
if(DOMAIN_67 && $domain){
    $CONFIG['cookie_domain'] = $domain['cookie_domain'];
}else{
